...
export function ErrorRateFieldBox({ classes = undefined }) {
    ...
    /* ERROR case */ if (nbrWords < nbrErrors || nbrWords < 0 || nbrErrors < 0) {
        ...
        grade.OutputField = (
            <OutputField value="Erreur" type="text" label={grade.label} classes="is-invalid" key={grade.key} />
        );
...
export function ComplianceRateFieldBox({ classes = undefined }) {
    ...
    const requiredElementsInputField = (
        <InputField setter={setRequiredElements} type="number" label="Nbr exigé" small={true} min="0" key="1" />
    );
    const placedElementsInputField = (
        <InputField setter={setPlacedElements} type="number" label="Nbr effectué" small={true} min="0" key="2" />
    );
    //#endregion
